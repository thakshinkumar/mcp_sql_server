#!/usr/bin/env python3
"""Test SQL generation with cleanup"""
...
print("Testing SQL Generation with Cleanup:")
print("=" * 80)

all_passed = True
for query, expected in test_queries:
    sql_generated = nl_to_sql_mock_test(query)
    sql_cleaned = clean_sql_output(sql_generated)
    passed = sql_cleaned == expected
    all_passed = all_passed and passed
    
    status = "✓" if passed else "✗"
    print(f"{status} Query:     {query}")
    print(f"  Generated: {sql_generated}")
    print(f"  Cleaned:   {sql_cleaned}")
